...
#import "DeMarcelpociotSmpagecontrolScrollableView.h"
#import "DeMarcelpociotSmpagecontrolScrollableViewProxy.h"
#import "TiUtils.h"
#import "TiViewProxy.h"

@interface DeMarcelpociotSmpagecontrolScrollableView(redefiningProxy)
@property(nonatomic,readonly)	DeMarcelpociotSmpagecontrolScrollableViewProxy * proxy;
@end

@implementation DeMarcelpociotSmpagecontrolScrollableView
...
-(void)dealloc
{
    ...
    RELEASE_TO_NIL(currentImages);
	RELEASE_TO_NIL(scrollview);
	RELEASE_TO_NIL(pageControl);
    RELEASE_TO_NIL(pageIndicatorMaskImage);
    RELEASE_TO_NIL(pageIndicatorImage);
    RELEASE_TO_NIL(currentPageIndicatorImage);
...
-(id)init
{
	if (self = [super init]) {
        ...
        pageControlMargin=20.0f;
        pageControlDiameter=10.0f;
        pageControlBackgroundColor = [[UIColor blackColor] retain];
        pageIndicatorTintColor = [[UIColor lightGrayColor] retain];
        currentPageIndicatorTintColor = [[UIColor whiteColor] retain];
        pageControlAlignment = SMPageControlAlignmentCenter;
...
-(SMPageControl*)pagecontrol
{
	if (pageControl==nil)
	{
		pageControl = [[SMPageControl alloc] initWithFrame:[self pageControlRect]];
        ...
        [pageControl setPageIndicatorImage:pageIndicatorImage];
        [pageControl setPageIndicatorMaskImage:pageIndicatorMaskImage];
        [pageControl setCurrentPageIndicatorImage:currentPageIndicatorImage];
        
        [pageControl setAlignment:pageControlAlignment];
...
-(void)refreshPageControl
{
	if (showPageControl)
	{
		SMPageControl *pg = [self pagecontrol];
        ...
        [pg setPageIndicatorImage:pageIndicatorImage];
        [pg setIndicatorMargin:pageControlMargin];
        [pg setIndicatorDiameter:pageControlDiameter];
        [self setImages_:images];
        [self setCurrentImages_:currentImages];
        [pg setPageIndicatorMaskImage:pageIndicatorMaskImage];
        [pg setCurrentPageIndicatorImage:currentPageIndicatorImage];
        [pg setAlignment:pageControlAlignment];
...
{
    if ([(DeMarcelpociotSmpagecontrolScrollableViewProxy *)[self proxy] viewCount] == 0) {
        return;
    }
...
-(void)setPagingControlAlignment_:(id)args
{
    NSString* val = [TiUtils stringValue:args];
    NSUInteger alignment = SMPageControlAlignmentCenter;
    if( [[val lowercaseString] isEqualToString:@"left"] ){
        alignment = SMPageControlAlignmentLeft;
    } else if ( [[val lowercaseString] isEqualToString:@"right"] ){
        alignment = SMPageControlAlignmentRight;
    }
    if (val != nil) {
        pageControlAlignment = alignment;
        if (showPageControl && (scrollview!=nil) && ([[scrollview subviews] count]>0)) {
            [[self pagecontrol] setAlignment:alignment];
        }
    }
}

-(void)setPagingControlMargin_:(id)args
{
    CGFloat val = [TiUtils floatValue:args];
    pageControlMargin = val;
    if (showPageControl && (scrollview!=nil) && ([[scrollview subviews] count]>0)) {
        [[self pagecontrol] setIndicatorMargin:pageControlMargin];
    }
}

-(void)setPagingControlDiameter_:(id)args
{
    CGFloat val = [TiUtils floatValue:args];
    pageControlDiameter = val;
    if (showPageControl && (scrollview!=nil) && ([[scrollview subviews] count]>0)) {
        [[self pagecontrol] setIndicatorDiameter:val];
    }
}
...
-(void)setPageIndicatorMaskImage_:(id)args
{
    UIImage* val = [TiUtils toImage:args proxy:self.proxy];
    if (val != nil) {
        RELEASE_TO_NIL(pageIndicatorMaskImage);
        pageIndicatorMaskImage = [val retain];
        if (showPageControl && (scrollview!=nil) && ([[scrollview subviews] count]>0)) {
            [[self pagecontrol] setPageIndicatorMaskImage:pageIndicatorMaskImage];
        }
    }
}

-(void)setPageIndicatorImage_:(id)args
{
    UIImage* val = [TiUtils toImage:args proxy:self.proxy];
    if (val != nil) {
        RELEASE_TO_NIL(pageIndicatorImage);
        pageIndicatorImage = [val retain];
        if (showPageControl && (scrollview!=nil) && ([[scrollview subviews] count]>0)) {
...
-(void)setImages_:(id)args
{
    ...
    ENSURE_ARRAY(args);
    images = [args retain];
    for( NSDictionary* arg in images ){
        UIImage* val    = [TiUtils toImage:[arg objectForKey:@"image"] proxy:self.proxy];
        int page        = [TiUtils intValue:[arg objectForKey:@"page"]];
        if (val != nil) {
            if (showPageControl && (scrollview!=nil) && ([[scrollview subviews] count]>0)) {
                [[self pagecontrol] setImage:val forPage:page];
            }
        }
    }
    }
...
-(void)setCurrentImages_:(id)args
{
    if( args != nil ){
    ENSURE_ARRAY(args);
    currentImages = [args retain];
    for( NSDictionary* arg in currentImages ){
        UIImage* val    = [TiUtils toImage:[arg objectForKey:@"image"] proxy:self.proxy];
        int page        = [TiUtils intValue:[arg objectForKey:@"page"]];
        if (val != nil) {
            if (showPageControl && (scrollview!=nil) && ([[scrollview subviews] count]>0)) {
                [[self pagecontrol] setCurrentImage:val forPage:page];
            }
        }
    }
    }
...
-(void)setCurrentPageIndicatorImage_:(id)args
{
    UIImage* val = [TiUtils toImage:args proxy:self.proxy];
    if (val != nil) {
        RELEASE_TO_NIL(currentPageIndicatorImage);
        currentPageIndicatorImage = [val retain];
        if (showPageControl && (scrollview!=nil) && ([[scrollview subviews] count]>0)) {
...
{
	int pageNum = [(SMPageControl *)sender currentPage];
